%macro AddVar(in_data= , in_var= , splitchar=~ , maxlen=200 , out_data= , out_pre= );
data _null_; 
...
/*Flag dataset*/ 
proc sql noprint; 
select distinct max(length(&in_var)) into :lngth from &in_data ; 
quit;
...
/*Number of variables*/ 
proc sql noprint; 
select cats(max(count(_&in_var._, "&splitchar"))-1) into :varn from &in_data ; 
quit;
...
%else %do; 
data &out_data; 
set &in_data(rename=COVAL=_COVAL_); 
length COVAL $200; 
COVAL=_COVAL_; 
drop _COVAL_; 
run; 
